...
for person in range(3):
    for dataset in range(3):
        ...
        if dataset == 2:
            ...
            plt.figure()
            plt.plot(angle_avg[person], emg_avg[person], color="black")
            plt.axvspan(90, angle_end[0], color='cyan', alpha=0.2)
            plt.axvspan(angle_end[0], angle_end[1], color='magenta', alpha=0.2)
            plt.axvspan(angle_end[1], angle_avg[person][len(angle_avg[person]) - 1], color='lime', alpha=0.2)
    ...
    if person == 2:
        ...
        plt.figure()
        plt.plot(angle_avg_all, emg_avg_all, color = "black")
        plt.axvspan(90, angle_end_all[0], color='cyan', alpha=0.2)
        plt.axvspan(angle_end_all[0], angle_end_all[1], color='magenta', alpha=0.2)
        plt.axvspan(angle_end_all[1], angle_avg_all[len(emg_avg_all) - 1], color='lime', alpha=0.2)
        plt.gca().invert_xaxis() # x-Achse mit abnehmenden Werten
